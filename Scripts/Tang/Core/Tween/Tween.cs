...
namespace Tang
{
    ...
    public class Tween
    {
        ...
        static float M_PI = Mathf.PI;
        static float M_PI_2 = Mathf.PI / 2.0f;
        static float M_PI_X_2 = Mathf.PI * 2.0f;

        public static float tweenChange(float time, TweenType type)
        {
            float precision = 0.01f;
            float ret = (tweenTo(time + precision, type) - tweenTo(time, type)) / precision;
            if (ret < 0.0f) ret = 0.0f;
            return ret;
        }

        public static float tweenTo(float time, TweenType type)
        {
            float[] easingParam = null;
            float delta = 0;
